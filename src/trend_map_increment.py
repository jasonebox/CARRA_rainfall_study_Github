...
for year_offset in range(17):
    ...
    iyear=year_offset+1991 ; fyear=1991+14+year_offset
    n_years=fyear-iyear+1
    
    years=np.arange(iyear,fyear+1).astype('str')
    
    print(year_offset,n_years,years[0],years[-1])
    ...
    if annotatex:
        ...
        plt.text(xx0, yy0+cc*dy2,varnam2, fontsize=font_size*mult,
                 transform=ax.transAxes,color=color_code) ; cc+=1.
        plt.text(xx0, yy0+cc*dy2,'Greenland trend', fontsize=font_size*mult,
                 transform=ax.transAxes,color=color_code) ; cc+=1.
