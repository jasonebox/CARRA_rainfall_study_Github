...
for yy in range(n_years):
    v=np.where((df.year==yy+iyear)&(df.month>=6)&(df.month<=8))
    v=v[0]
...
GBI=np.array(GBI)
GBI=(GBI-np.mean(GBI))/np.std(GBI)
GBIs=pd.DataFrame({'year':years.astype(int),'GBI':GBI})
GBIs=GBIs[GBIs.year>1990]
GBIs=GBIs[GBIs.year<2022]
# print(GBI)

GBIs = GBIs.reset_index(drop=True)
...
value=1.
if GBI_phase=='GBI+':
    v=np.where(GBIs.GBI.values>value)
if GBI_phase=='GBI-':
    v=np.where(GBIs.GBI.values<-value)

iyear=1991 ; fyear=2021
n_years=fyear-iyear+1
yearsx=np.arange(iyear,fyear+1).astype('str')
years=yearsx[v[0]]
print(years)
#%%
n_years=len(years)
...
cm='bwr'
meanx.astype(dtype=np.float16).tofile('/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_average_'+GBI_phase+'_1269x1069.npy')
...
varnam='tp'
th=1
fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_average_'+GBI_phase+'_1269x1069.npy'
...
if annotatex:
    ...
    plt.text(xx0, yy0+cc*dy2,GBI_phase, fontsize=font_size*mult,
             transform=ax.transAxes,color=color_code) ; cc+=1.
...
if tight_layout:
    ...
    if ly =='p':
      ...
      # os.system('open /tmp/t.png')
        ...
        fn=figpath+varnam+'_GBI_composite_'+GBI_phase+'.png'
        out_im.save(fn)  
        os.system('open '+fn)
else:
        figpath='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/Figs/'
        figname=figpath+varnam+'_GBI_composite_'+GBI_phase+'.png'
