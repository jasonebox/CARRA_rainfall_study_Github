...
# for i in range(1):
    if i==2:
        ...
        for yy,year in enumerate(years):
            print(year)
            fn=inpath+varnams[i]+'_'+year+'_'+str(ni)+'x'+str(nj)+'_float16.npy'
...
i=2
anom_year='2010'
fn=inpath+varnams[i]+'_'+anom_year+'_'+str(ni)+'x'+str(nj)+'_float16.npy'
t=np.fromfile(fn, dtype=np.float16)
t=t.reshape(ni, nj)
t=np.rot90(t.T)
...
plt.imshow(anom,vmin=lo,vmax=-lo,cmap='bwr')
plt.grid('Off')
plt.colorbar()
                    #%%
                # plt.imshow(plotvar)
