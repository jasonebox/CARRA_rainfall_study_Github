...
statvar=np.zeros(((n_years,ni,nj)))
for yy,year in enumerate(years):
    # print(yy,year)
    fn='/Users/jason/Dropbox/CARRA/CARRA_rain/output_annual/'+varnam+'_'+year+'_1269x1069_float16.npy'
    print(yy,year,fn)
    t=np.fromfile(fn, dtype=np.float16)
    t=t.reshape(ni, nj)
    t=np.rot90(t.T)
    # plt.imshow(t)
    statvar[yy,:,:]=t[:,:]
...
slope=np.zeros((ni,nj))
confidence=np.zeros((ni,nj))

x=np.arange(0,n_years)

for i in range(ni):
    print(iyear,fyear,ni-i)
    for j in range(nj):
        # if mask[i,j]>0:
        y=statvar[:,i,j]
        # print(x,y)
...
#%% obtain mean
print('obtain mean')
meanx=np.nanmean(statvar,axis=0)
# print(np.shape(meanx))
# plt.imshow(meanx,vmin=0,vmax=maxval)
# plt.axis('off')
# plt.colorbar()
...
# temp=slope*n_years
# temp/=meanx ; lo=-1 ; hi=-lo
# temp[confidence<0.8]=np.nan
# temp[meanx<5]=np.nan
# temp[(confidence<0.9)or(meanx<5)]=np.nan
# cm='bwr'
# temp=meanx ; lo=0 ; hi=60 ; cm='viridis'
# temp/=meanx ; lo=-3 ; hi=-lo
# temp[mask==0]=np.nan
# plt.imshow(temp,cmap=cm,vmin=lo,vmax=hi)

# clb = plt.colorbar()
# clb.set_label('mm', labelpad=-40, y=1.05, rotation=0)
# plt.colorbar(set_title='mm')
# plt.axis('off')
# temp.astype(dtype=np.float16).tofile('/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_change_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy')
# temp2=slope.copy()
# temp2[mask==0]=np.nan
slope.astype(dtype=np.float16).tofile('/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_slope_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy')
meanx.astype(dtype=np.float16).tofile('/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_average_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy')
confidence.astype(dtype=np.float16).tofile('/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_confidence_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy')
...
for varnam in varnams:
    th=1
    if varnam!='sf':
        fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_slope_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy'
        slope=np.fromfile(fn, dtype=np.float16)
        slope=slope.reshape(ni, nj)
    else:
        fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/tp_slope_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy'
        slopea=np.fromfile(fn, dtype=np.float16)
        fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/rf_slope_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy'
        slopeb=np.fromfile(fn, dtype=np.float16)
        slope=slopea-slopeb
        slope=slope.reshape(ni, nj)
    # slope=np.rot90(slope.T)
    plotvar=slope*n_years
    # plt.imshow(plotvar)
    ...
    if varnam=='t2m':
        lo=-5 ; hi=-lo ; dx=0.1
        varnam2='air temperature'
        maxval=60
        
    fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_confidence_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy'
    confidence=np.fromfile(fn, dtype=np.float16)
    confidence=confidence.reshape(ni, nj)
    # slope=np.rot90(slope.T)
    # plotvar=confidence
    
    fn='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/output/'+varnam+'_average_'+str(iyear)+'-'+str(fyear)+'_1269x1069.npy'
    meanx=np.fromfile(fn, dtype=np.float16)
    meanx=meanx.reshape(ni, nj)
    
    # do_confidence_mask=0
    # if do_confidence_mask:
    #     plotvar[confidence<0.66]=np.nan
    ...
    if annotatex:
        ...
        color_code='k'
        print()
        plt.text(xx0, yy0+cc*dy2,'CARRA annual', fontsize=font_size*mult,
                 transform=ax.transAxes,color=color_code) ; cc+=1.
        plt.text(xx0, yy0+cc*dy2,varnam2, fontsize=font_size*mult,
                 transform=ax.transAxes,color=color_code) ; cc+=1.
        plt.text(xx0, yy0+cc*dy2,'change', fontsize=font_size*mult,
                 transform=ax.transAxes,color=color_code) ; cc+=1.
    ...
    if tight_layout:
        ...
        if ly =='p':
          ...
          # os.system('open /tmp/t.png')
            ...
            figpath='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/Figs/trend_maps/'
            fn=figpath+varnam+'_trend_map_'+str(iyear)+'-'+str(fyear)+confidence_name+'.png'
            out_im.save(fn)  
            os.system('open '+fn)
    else:
            figpath='/Users/jason/Dropbox/CARRA/CARRA_rainfall_study/Figs/trend_maps/'
            figname=figpath+varnam+'_trend_map_'+str(iyear)+'-'+str(fyear)+confidence_name+'.png'
